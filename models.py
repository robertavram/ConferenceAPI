...
class WishList(ndb.Model):
    ...
    def to_form(self):
        ...
        sessions_out = mm.ConferenceSessionForms(items=[sessions[i].to_form(speakers[i]) for i in range(len(sessions))])
        ...
        return mm.WishListForm(conferences=conferences_out, sessions=sessions_out)
...
class ConferenceSession(ndb.Model):
    ...
    def from_form(cls, mys, parent_key):
        ...
        REQUIRED_FIELDS =['name', 'startTime', 'startDate', 'duration', 'speakerKey']
        data = {field.name: getattr(mys, field.name) for field in mys.all_fields()}
        ...
        if error:
            raise endpoints.BadRequestException(', '.join(error) + ' are required')
