...
# - - - Announcements - - - - - - - - - - - - - - - - - - - -
    ...
    def getAnnouncement(self, request):
        ...
        return mm.StringMessage(data=memcache.get(MEMCACHE_ANNOUNCEMENTS_KEY) or "")
...
### by @Robert_Avram: - - - - - - - - - - - - -- - - - - - - - - - - - - - - - - - -
    ...
    def getConferenceSessions(self, request):
        ...
        # get the conference
        confKey = self.get_websafe_key(request.websafeConferenceKey, "Conference")
        conf = confKey.get()
        if not conf:
            raise endpoints.NotFoundException("The conference you are looking for does not exist")
    ...
    def getConferenceSessionsByType(self, request):
        ...
        if len(request.typeOfSession)>5:
            raise endpoints.BadRequestException("Maximum number of typeOfSession to include is 5")
        
        # get the conference
        confKey = self.get_websafe_key(request.websafeConferenceKey, "Conference")
        conf = confKey.get()
        if not conf:
            raise endpoints.BadRequestException("This conference does not exist: %s"%confKey)
    ...
    def getSessionsBySpeaker(self, request):
        ...
        speaker_key = self.get_websafe_key(request.websafeSpeakerKey, "ConferenceSpeaker")
        speaker = speaker_key.get()
        if not speaker:
            raise endpoints.NotFoundException("The speaker you are looking for was not found!")
        
        sessions = ndb.get_multi(speaker.conferenceSessions)
        
        return mm.ConferenceSessionForms(items=[sessions[i].to_form(speaker) for i in range(len(sessions))])
    ...
    def getSpeakers(self, unused_request):
        ...
        return mm.ConferenceSpeakerForms(items=[speaker.to_form() for speaker in speakers])
    ...
    def getSpeakersByName(self, request):
        ...
        speakers = ConferenceSpeaker.query(ConferenceSpeaker.displayName == request.displayName).fetch(10)
        return mm.ConferenceSpeakerForms(items=[speaker.to_form() for speaker in speakers])
    ...
    def removeSessionFromWishList(self, request):
        ...
        return mm.BooleanMessage(data = self._remove_session_from_wishlist(request.websafeSessionKey, request.removeConference))
    ...
    def queryproblem2(self, request):
        ...
        return mm.ConferenceSessionForms_search(items = self._queryproblem2(request))
    ...
    def getConferencesBySpeaker(self, request):
        ...
        return mm.ConferenceForms(items=[conf.to_form(None) for conf in conferences])
    ...
    def getSessionsFromSpeakerAndConference(self, request):
        ...
        conf_key = self.get_websafe_key(request.websafeConferenceKey, "Conference")
        conference = conf_key.get()
        if not conference:
            raise endpoints.NotFoundException("The conference you are looking for was not found!")
